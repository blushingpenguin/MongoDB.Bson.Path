...
namespace MongoDB.Bson.Path
{
    internal class ArraySliceFilter : PathFilter
    {
        ...
        public override IEnumerable<BsonValue> ExecuteFilter(BsonValue root, IEnumerable<BsonValue> current, bool errorWhenNoMatch)
        {
            if (Step == 0)
            {
                throw new BsonException("Step cannot be zero.");
            }
            ...
            foreach (BsonValue t in current)
            {
                if (t is BsonArray a)
                {
                    ...
                    else
                    {
                        if (errorWhenNoMatch)
                        {
                            throw new BsonException("Array slice of {0} to {1} returned no results.".FormatWith(CultureInfo.InvariantCulture,
                ...
                else
                {
                    if (errorWhenNoMatch)
                    {
                        throw new BsonException("Array slice is not valid on {0}.".FormatWith(CultureInfo.InvariantCulture, t.GetType().Name));
                    }
                }
            }
        }
